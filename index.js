...
const MAX_CHECK_INTERVAL = 60000 // Don't check more often than every 1 minute
...
const limiter = new Bottleneck({
  ...
  minTime: 2000 // Wait at least 2 second between each request
})
...
async function searchThreads(board, searchTerm) {
  try {
    ...
    // Most of this can be ignored used for debugging and was not needed
    if (error.response) {
      // The request was made, and the server responded with a status code
      // that falls out of the range of 2xx
      logWithTimestamp(`Error fetching catalog data: ${error.response.status}`)
      logWithTimestamp(`Response data: ${JSON.stringify(error.response.data)}`)
    } else if (error.request) {
      // The request was made but no response was received
      logWithTimestamp(`Error fetching catalog data: No response received`)
      logWithTimestamp(`Request: ${JSON.stringify(error.request)}`)
    } else {
      // Something happened in setting up the request that triggered an Error
      logWithTimestamp(`Error fetching catalog data: ${error.message}`)
    }
    return []
  }
}
...
`;
  ...
  html += `
  <small>Generated by https://github.com/TJBK/4Chan-Scraper</small>
  </body>
  </html>
  `;
    ...
    if (response.status === 200) {
      ...
      const updateThread = async () => {
        ...
        if (response.status === 200) {
          ...
          if (dynamicCheckInterval !== MAX_CHECK_INTERVAL) {
            dynamicCheckInterval *= 2
            logWithTimestamp(`Thread ${threadId} not modified. Increasing check interval to ${dynamicCheckInterval}ms`)
          }
          logWithTimestamp(`Thread ${threadId} not modified. Next check in ${dynamicCheckInterval}ms`)
        }
